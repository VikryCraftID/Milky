export default {
    command: 'warn',
    category: 'group',
    get description() { return L('plugins.group.warnDesc'); },
    syntax: 'warn <tag/reply/number>',
    subcommand: '',
    aliases: ['warning'],
    async run(context) {
        ...
        if (!db.groups[m.chat].warn[nmrnya]) {
            ...
            reply(L('plugins.group.warnProgress', 1));
        } else if (db.groups[m.chat].warn[nmrnya] >= 3) {
            await naze.groupParticipantsUpdate(m.chat, [nmrnya], 'remove').catch(() => reply(global.mess.fail));
            delete db.groups[m.chat].warn[nmrnya];
            reply(L('plugins.group.warnKick'));
        } else {
            db.groups[m.chat].warn[nmrnya] += 1;
            reply(L('plugins.group.warnProgress', db.groups[m.chat].warn[nmrnya]));
        }
    }
};